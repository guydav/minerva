...
class School(peewee.Model):
    ...
    def __repr__(self):
        contact = self.get_identifier()
        return '{contact} - {name} ({state})'.format(contact=contact, name=self.name, state=self.state)
        ...
        if emails:
            ...
            for email in emails:
                ...
                if supervisor:
                    ...
                    priority = email_heuristic(name_split[0], name_split[1], email)
...
HEADERS = ('Organization', 'First', 'Last', 'School', 'Title', 'State', 'Key Club Code', 'Top Email', 'Top Ten Emails')
KEY_CLUB = 'Key Club'
TITLE = 'Advisor'
MAXIMUM_EMAILS = 100
...
def write_schools(output_name):
    ...
    for school in School.select():
        ...
        row = (KEY_CLUB, first, last, school.name, TITLE, school.state, school.club_code,
                         email_pq.get(min=False), ', '.join([email_pq.get(min=False) for i in xrange(10)]))
